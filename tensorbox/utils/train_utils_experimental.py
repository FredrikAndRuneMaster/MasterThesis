...
def load_data_gen(H, phase, jitter):
    ...
    data = load_idl_tf(H["data"]['%s_idl' % phase], H, jitter={'train': jitter, 'test': False, 'evaluate': False}[phase])
...
def add_rectangles_confidence(H, orig_image, confidences, boxes, use_stitching=False, rnn_len=1, min_conf=0.1, show_removed=True, tau=0.25):
    image = np.copy(orig_image[0])
    num_cells = H["grid_height"] * H["grid_width"]
    boxes_r = np.reshape(boxes, (-1,
                                 H["grid_height"],
    ...
    for x in all_rects:
        for y in x:
            for z in y:
                if z.confidence > best_confidence:
                    best_confidence = z.confidence 

    return image, rects, best_confidence
